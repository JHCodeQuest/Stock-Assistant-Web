...
export const matchImage = async (
  ...
  try {
    ...
    const result = await pool.query(
      `SELECT * FROM inventory WHERE image_url IS NOT NULL AND (
       LOWER(name) LIKE ANY($1) OR 
       LOWER(category) LIKE ANY($1) OR
       LOWER(COALESCE(description, '')) LIKE ANY($1)
...
export const updateStock = async (
  ...
  try {
    ...
    const itemResult = await pool.query('SELECT quantity FROM inventory WHERE id = $1', [productId]);
    ...
    await pool.query(
      `INSERT INTO inventory_history (product_id, adjustment_type, quantity, previous_quantity, reason)
       VALUES ($1, $2, $3, $4, $5)`,
      [productId, adjustmentType, quantity, previousQuantity, reason]
    );
...
export const getLowStockItems = async (
  ...
  try {
    const result = await pool.query(
      'SELECT * FROM inventory WHERE quantity <= min_stock_level ORDER BY quantity'
    );
    res.json(result.rows);
  } catch (error) {
